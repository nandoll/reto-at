...
export default function ProfileBetCard({ bet }: ProfileBetCardProps) {
  ...
  return (
    <Link
      href={`/bets/${bet.id}`}
      className={`block rounded-[var(--radius-lg)] ${config.bg} p-3.5 transition-shadow hover:shadow-md lg:p-5`}
    >
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-1.5">
          {/* Mobile: text only, no crests */}
          <span
            className={`text-[13px] font-semibold lg:hidden ${config.textMuted ? "text-text-tertiary" : "text-text-primary"}`}
          >
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            />
            <span
              className={`text-sm font-bold ${config.textMuted ? "text-text-tertiary" : "text-text-primary"}`}
            >
              {bet.homeTeam} vs {bet.awayTeam}
            </span>
          </div>
          <span className="text-[11px] text-text-tertiary">
            {pickedTeam} ({pickLabels[bet.pick]}) · {bet.odd.toFixed(2)} · S/.
            {bet.stake.toFixed(0)}
          </span>
        </div>
        <span
      ...
      <div className="mt-2 lg:mt-4">
        {bet.status === "WON" && bet.return !== null && (
          <span className="text-xs font-bold text-gain-positive lg:text-sm">
        ...
        {bet.status === "LOST" && (
          <span className="text-xs text-text-tertiary lg:text-sm">
