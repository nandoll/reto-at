...
export default function ProfileBetList({ seedBets }: ProfileBetListProps) {
  ...
  const stats = {
    ...
    winRate:
      counts.total > 0 ? Math.round((counts.WON / counts.total) * 100) : 0,
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex gap-1.5 overflow-x-auto lg:gap-2">
        {filters.map((f) => (
          <button
            key={f.label}
            onClick={() => setStatusFilter(f.value)}
            className={`flex shrink-0 items-center gap-1 rounded-full px-3 py-1.5 text-[11px] font-semibold transition-all lg:gap-1.5 lg:px-4 lg:text-xs ${
              statusFilter === f.value
                ? "bg-primary text-white"
                : "bg-odd-default-bg text-text-secondary lg:border lg:border-border-medium lg:bg-surface"
            }`}
          >
            {f.label}
            <span
              className={`text-[10px] ${statusFilter === f.value ? "text-white/70" : "text-text-tertiary"}`}
            >
              {f.count}
            </span>
          </button>
